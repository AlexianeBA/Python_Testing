...
class TestDeductPoints:
    ...
    def test_deduct_points(self):
        ...
        places_booked = 2
        result = self.client.post('/purchasePlaces', data={
            "places": places_booked, "club": self.club[0]["name"],
            "competition": self.competition[0]["name"]
        })
        assert result.status_code == 200
        assert "Great-booking complete!" in result.data.decode()
        assert int(self.club[0]["points"]) == club_points_before_reserved - places_booked
